...
elseif Flag_grid == 2
    ...
    xi_gd = linspace(0, length_x, nx);    % horizontal positions
    zt_gd = linspace(0, length_z, nz);    % depth levels
    ...
    if Grid_disp==1
        ...
        for i = 1:nx
            plot(x_gd(:,i), z_gd(:,i), 'k');  % black
        end

        % Plot horizontal lines (along X)
        for j = 1:nz
            if j <= 3  % Top 3 horizontal layers
                plot(x_gd(j,:), z_gd(j,:), 'r', 'LineWidth', 1.5);  % red
            else
                plot(x_gd(j,:), z_gd(j,:), 'k');  % black
            end
        end
...
if Grid_disp==1
    ...
    shading interp;  % 或 shading flat 更清晰
    axis equal tight;
    xlabel('x (m)');
    ylabel('z (m)');
